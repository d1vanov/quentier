...
{
    QNDEBUG("MainWindowSideBordersController::"
...
void MainWindowSideBordersController::persistCurrentBordersSizes()
{
    ...
    {
        QNWARNING("Internal error: can't persist the current main "
                  << "window's left & right borders size: expected 5 "
                  << "items within the horizontal splitter but got "
                  << splitterSizesCount);
        return;
    }
    ...
    {
        QNINFO("Couldn't restore the persistent "
    ...
    QNDEBUG("Show left border option = " << leftBorderOption
            << ", left border is displayed = "
            << (leftBorderIsDisplayed ? "true" : "false")
            << ", show right border option = " << rightBorderOption
            << ", right border is displayed = "
            << (rightBorderIsDisplayed ? "true" : "false")
            << ", main window is maximized = "
            << (mainWindowIsMaximized ? "true" : "false")
            << ", left border splitter size = " << splitterSizes[0]
            << ", right border splitter size = " << splitterSizes[4]);
...
{
    QNDEBUG("MainWindowSideBordersController::"
            << "onShowLeftBorderOptionChanged: " << option);
...
{
    QNDEBUG("MainWindowSideBordersController::"
            << "onShowRightBorderOptionChanged: " << option);
...
{
    QNDEBUG("MainWindowSideBordersController::"
            << "onLeftBorderWidthChanged: " << width);
...
{
    QNDEBUG("MainWindowSideBordersController::"
            << "onRightBorderWidthChanged: " << width);
...
{
    QNDEBUG("MainWindowSideBordersController::"
            << "onLeftBorderColorChanged: color code = " << colorCode);
...
{
    QNDEBUG("MainWindowSideBordersController::"
            << "onRightBorderColorChanged: color code = " << colorCode);
    onBorderColorChanged(colorCode, m_rightBorder);
}

void MainWindowSideBordersController::onMainWindowMaximized()
{
    QNDEBUG("MainWindowSideBorderOption::onMainWindowMaximized");
    initializeBordersState();
}

void MainWindowSideBordersController::onMainWindowUnmaximized()
{
    QNDEBUG("MainWindowSideBordersController::onMainWindowUnmaximized");
    initializeBordersState();
}

void MainWindowSideBordersController::setCurrentAccount(const Account & account)
{
    QNDEBUG("MainWindowSideBordersController::setCurrentAccount: " << account);

    if (m_currentAccount == account) {
        QNDEBUG("This account is already set, nothing to do");
        return;
    }
...
{
    QNDEBUG("MainWindowSideBordersController::onPanelStyleChanged: "
            << panelStyle);
...
void MainWindowSideBordersController::onHideBorderRequestFromContextMenu()
{
    ...
    if (Q_UNLIKELY(!pAction)) {
        QNWARNING("Can't hide the main window border: internal "
...
void MainWindowSideBordersController::onHideBorderWhenNotMaximizedRequestFromContextMenu()
{
    ...
    QAction * pAction = qobject_cast<QAction*>(sender());
    if (Q_UNLIKELY(!pAction)) {
        QNWARNING("Can't hide the main window border when it is "
...
void MainWindowSideBordersController::toggleBorderDisplay(
    const int option, QWidget & border)
{
    MainWindowSideBorderOption::type borderOption =
        DEFAULT_SHOW_MAIN_WINDOW_BORDER_OPTION;
    if ((option < 0) || (option > 2)) {
        QNWARNING("Can't process the toggle border display option, "
...
void MainWindowSideBordersController::onBorderWidthChanged(
    const int width, QWidget & border)
{
    QNDEBUG("MainWindowSideBordersController::onBorderWidthChanged: width = "
            << width);
    ...
    if (Q_UNLIKELY(splitterSizesCount != 5))
    {
        QNWARNING("Internal error: can't set up the proper border "
                  << "width: expected 5 items within the "
    ...
    if (widthDiff == 0) {
        QNDEBUG("Border's width hasn't actually changed, nothing to do");
        return;
    }

    if (Q_UNLIKELY(std::abs(widthDiff) >= splitterSizes[3])) {
        QNWARNING("Detected inappropriate change in the width of the border: "
                  "the diff is larger than the supposedly largert part of the "
                  "main window's horizontal layout");
        return;
    }

    QNDEBUG("Original left border width = " << splitterSizes[0]
            << ", original right border width = " << splitterSizes[4]
            << ", original note editor width = " << splitterSizes[3]);
    ...
    QNDEBUG("New left border width = " << splitterSizes[0]
            << ", new right border width = " << splitterSizes[4]
            << ", new note editor width = " << splitterSizes[3]);

    m_horizontalLayoutSplitter.setSizes(splitterSizes);
}

void MainWindowSideBordersController::onBorderColorChanged(
    const QString & colorCode, QWidget & border)
{
...
void MainWindowSideBordersController::setBorderStyleSheet(
    const QString & colorCode, QWidget & border)
{
...
{
    QNDEBUG("MainWindowSideBordersController::initializeBordersState");
    ...
    if (!conversionResult ||
        ...
        (leftBorderOptionInt > 2))
    {
        QNINFO("Couldn't restore the persistent \"Show left main window "
               "border\" option, using the default one");
    }
    else {
        leftBorderOption =
            static_cast<MainWindowSideBorderOption::type>(leftBorderOptionInt);
    ...
    if (!conversionResult ||
        ...
        (rightBorderOptionInt > 2))
    {
        QNINFO("Couldn't restore the persistent \"Show right main window "
               "border\" option, using the default one");
    }
    else {
        rightBorderOption =
    ...
    if (Q_UNLIKELY(!conversionResult)) {
        QNDEBUG("Couldn't restore the persistent \"Left main window border "
                "width\" option, using the default one");
        leftBorderWidth = 4;
    }
    ...
    int rightBorderWidth =
        ...
        .toInt(&conversionResult);
    if (Q_UNLIKELY(!conversionResult)) {
        QNDEBUG("Couldn't restore the persistent \"Right main window border "
                "width\" option, using the default one");
        rightBorderWidth = 4;
    }
    ...
    {
        QNDEBUG("Original left border size = " << splitterSizes[0]
                << ", original right border size = " << splitterSizes[4]
                << ", original note editor size = " << splitterSizes[3]
                << ", new left border width = " << leftBorderWidth
                << ", new right border width = " << rightBorderWidth);
        ...
        QNDEBUG("Left border splitter size after: " << splitterSizes[0]
                << ", right border splitter size after: " << splitterSizes[4]
                << ", note editor splitter size after: " << splitterSizes[3]);
