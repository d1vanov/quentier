...
{
    QNTRACE("NotebookModel::setAccount: " << account);
...
QModelIndex NotebookModel::indexForItem(INotebookModelItem * pItem) const
{
    ...
    INotebookModelItem * pParentItem = pItem->parent();
    if (!pParentItem) {
    ...
    const QString & stack, const QString & linkedNotebookGuid) const
{
    ...
    {
        auto it = m_modelItemsByStackByLinkedNotebookGuid.find(
            linkedNotebookGuid);
    ...
    INotebookModelItem * pModelItem = it.value();
    ...
    const QModelIndex & index, const QString & stack)
{
    ...
    if (pModelItem->type() != INotebookModelItem::Type::Notebook) {
        REPORT_ERROR(
            QT_TR_NOOP("Can't move the non-notebook model item to a stack"));
        return QModelIndex();
    }

    NotebookItem * pNotebookItem = dynamic_cast<NotebookItem*>(pModelItem);
    if (Q_UNLIKELY(!pNotebookItem)) {
        REPORT_ERROR(
            QT_TR_NOOP("Internal error: found a notebook model item of "
                       "notebook type but cannot cast it to NotebookItem"));
        return QModelIndex();
    }
    ...
    INotebookModelItem * pParentItem = pModelItem->parent();
    ...
    INotebookModelItem * pStackParentItem = nullptr;
    ...
    if (notebookItem.linkedNotebookGuid().isEmpty())
    {
        ...
        pModelItemsByStack = &m_modelItemsByStack;
        pStackParentItem = m_pAllNotebooksRootItem;
    }
    else
    {
        ...
        auto modelItemsByStackIt = m_modelItemsByStackByLinkedNotebookGuid.find(
            linkedNotebookGuid);

        if (modelItemsByStackIt ==
            m_modelItemsByStackByLinkedNotebookGuid.end())
        {
            modelItemsByStackIt = m_modelItemsByStackByLinkedNotebookGuid.insert(
                linkedNotebookGuid,
                ModelItems());
        }

        auto parentItemIt = m_modelItemsByLinkedNotebookGuid.find(
            linkedNotebookGuid);

        if (Q_UNLIKELY(parentItemIt == m_modelItemsByLinkedNotebookGuid.end())) {
            REPORT_ERROR(
                QT_TR_NOOP("Internal error: failed to find the notebook model "
                           "items by notebook stack for a linked notebook"));
            return QModelIndex();
        }
    ...
    if (it == pModelItemsByStack->end()) {
        REPORT_ERROR(
            QT_TR_NOOP("Internal error: no notebook model item while "
    ...
    INotebookModelItem * pNewParentItem = *it;
...
QModelIndex NotebookModel::removeFromStack(const QModelIndex & index)
{
    ...
    INotebookModelItem * pNewParentItem =
        (linkedNotebookGuid.isEmpty()
    ...
    ErrorString & errorDescription)
{
    ...
    checkAndCreateModelRootItems();
    INotebookModelItem * pParentItem = m_pAllNotebooksRootItem;
    ...
    if (!notebookStack.isEmpty())
    {
        ...
        if (stackItemIt == m_stackItems.end())
        {
            ...
            stackItemIt = m_stackItems.insert(
                notebookStack,
        ...
        if (it == m_modelItemsByStack.end())
        {
            ...
            {
                QNWARNING("Internal error: no notebook model item "
                    << "while it's expected to be here; will try to auto-fix "
                    "it and add the new model item");
        ...
        {
            errorDescription.setBase(
                QT_TR_NOOP("Internal error: no notebook model item while it's "
                           "expected to be there; failed to auto-fix "
                           "the problem"));
        ...
        QNDEBUG("Will put the new notebook under parent item: "
            << *pParentItem);
    }
    ...
    auto modelItemInsertionResult =
        m_modelItemsByLocalUid.insert(item.localUid(), pAddedNotebookItem);
    ...
    const QString & itemName, const QString & linkedNotebookGuid) const
{
    ...
    INotebookModelItem * pModelItem = itemForIndex(index);
    if (!pModelItem) {
    ...
    if (Q_UNLIKELY(pModelItem->type() != INotebookModelItem::Type::Notebook)) {
    ...
    if (Q_UNLIKELY(!pNotebookItem)) {
        QNDEBUG("Found notebook model item of notebook type but "
            << "cannot cast it to NotebookItem: " << *pModelItem);
        return QString();
    }
...
Qt::ItemFlags NotebookModel::flags(const QModelIndex & index) const
{
    ...
    INotebookModelItem * pItem = itemForIndex(index);
    if (Q_UNLIKELY(!pItem)) {
    ...
    if (pItem->type() == INotebookModelItem::Type::Stack)
    {
        NotebookStackItem * pStackItem = dynamic_cast<NotebookStackItem*>(pItem);
        if (Q_UNLIKELY(!pStackItem)) {
            QNWARNING("Internal inconsistency detected: notebook model item "
                << "has stack type but cannot cast it to NotebookStackItem");
            return indexFlags;
        }
        ...
        QList<INotebookModelItem*> children = pItem->children();
        for(auto it = children.begin(), end = children.end(); it != end; ++it)
        {
            INotebookModelItem * pChildItem = *it;
            ...
            {
                QNWARNING("Detected nested notebook model item of non-notebook "
                    << "type (" << pChildItem->type() << "); it is unexpected "
                    << "and most likely incorrect");
                continue;
            }

            NotebookItem * pNotebookItem =
                dynamic_cast<NotebookItem*>(pChildItem);
            if (Q_UNLIKELY(!pNotebookItem)) {
                QNWARNING("Internal inconsistency detected: notebook model item "
                    << "has notebook type but cannot cast it to NotebookItem");
                continue;
            }
    ...
    else if (pItem->type() == INotebookModelItem::Type::Notebook)
    {
        NotebookItem * pNotebookItem = dynamic_cast<NotebookItem*>(pItem);
        if (Q_UNLIKELY(!pNotebookItem)) {
            QNWARNING("Internal inconsistency detected: notebook model item "
                << "has notebook type but cannot cast it to NotebookStackItem");
            return indexFlags;
        }
...
int NotebookModel::rowCount(const QModelIndex & parent) const
{
    ...
    INotebookModelItem * pParentItem = itemForIndex(parent);
    ...
    int row, int column, const QModelIndex & parent) const
{
    ...
    INotebookModelItem * pItem = pParentItem->childAtRow(row);
    if (!pItem) {
        return QModelIndex();
    }
...
QModelIndex NotebookModel::parent(const QModelIndex & index) const
{
    ...
    INotebookModelItem * pChildItem = itemForIndex(index);
    if (!pChildItem) {
        return QModelIndex();
    }

    INotebookModelItem * pParentItem = pChildItem->parent();
    if (!pParentItem) {
        return QModelIndex();
    }
    ...
    INotebookModelItem * pGrandParentItem = pParentItem->parent();
    if (!pGrandParentItem) {
        return QModelIndex();
    }
    ...
    const QModelIndex & modelIndex, const QVariant & value, int role)
{
    ...
    if (modelIndex.column() == static_cast<int>(Column::FromLinkedNotebook)) {
        REPORT_ERROR(
            QT_TR_NOOP("The \"from linked notebook\" flag can't be set manually"));
        return false;
    }

    if (modelIndex.column() == static_cast<int>(Column::NumNotesPerNotebook)) {
        REPORT_ERROR(
            QT_TR_NOOP("The \"notes per notebook\" column can't be set manually"));
        return false;
    }

    INotebookModelItem * pModelItem = itemForIndex(modelIndex);
    if (!pModelItem) {
        REPORT_ERROR(
    ...
    NotebookItem * pNotebookItem = nullptr;
    NotebookStackItem * pStackItem = nullptr;

    bool isNotebookItem =
        (pModelItem->type() == INotebookModelItem::Type::Notebook);

    if (isNotebookItem) {
        pNotebookItem = dynamic_cast<NotebookItem*>(pModelItem);
    }
    else {
        pStackItem = dynamic_cast<NotebookStackItem*>(pModelItem);
    }

    if (Q_UNLIKELY(isNotebookItem && !pNotebookItem)) {
        REPORT_ERROR(
            QT_TR_NOOP("Internal inconsistency detected in the notebook "
                       "model: the model item of notebook type "
                       "cannot be casted to NotebookItem"));
        return false;
    }

    if (Q_UNLIKELY(!isNotebookItem && !pStackItem)) {
        REPORT_ERROR(
            QT_TR_NOOP("Internal inconsistency detected in the notebook "
                       "model: the model item of stack type "
                       "cannot be casted to NotebookStackItem"));
        return false;
    }
    ...
    if (isNotebookItem)
    {
        ...
        else if ((modelIndex.column() == static_cast<int>(Column::Name)) &&
...
#define CHECK_ITEM(pChildItem)                                                 \
            ...
            {                                                                  \
                QNWARNING("Internal inconsistency detected: found "            \
                    << "non-notebook item being a child of "                   \
                    << "a notebook stack item: type = " << pChildItem->type()  \
                    << ", item = " << *pChildItem);                            \
                continue;                                                      \
            }                                                                  \
            NotebookItem * pNotebookItem =                                     \
                dynamic_cast<NotebookItem*>(pChildItem);                       \
            if (Q_UNLIKELY(!pNotebookItem)) {                                  \
                QNWARNING("Internal inconsistency detected: found item of "    \
                    << "notebook type within the children of notebook stack "  \
                    << "item but cannot cast it to NotebookItem: "             \
                    << *pChildItem);                                           \
                continue;                                                      \
            }                                                                  \
// CHECK_ITEM

        QList<INotebookModelItem*> children = pModelItem->children();
        ...
        {
            ErrorString error(
                QT_TR_NOOP("Internal error: can't update "
            ...
            (pParentItem->type() == INotebookModelItem::Type::LinkedNotebook))
        {
            auto * pLinkedNotebookItem =
                dynamic_cast<NotebookLinkedNotebookRootItem*>(pParentItem);
        ...
        NotebookStackItem newStackItem(*pStackItem);
            ...
            end = children.constEnd(); it != end; ++it)
        {
            ...
            if (Q_UNLIKELY(row < 0))
            {
                QNWARNING("Couldn't find row for the child notebook item of "
                    << "the stack item about to be removed; stack item: "
        ...
        stackModelItemIt = m_modelItemsByStack.insert(
            newStack,
            &(stackItemIt.value()));
...
bool NotebookModel::insertRows(int row, int count, const QModelIndex & parent)
{
    ...
    if (pParentItem->type() == INotebookModelItem::Type::Notebook) {
        QNDEBUG("Can't insert notebook under another notebook, "
            << "only under the notebook stack");
        return false;
    }

    auto * pStackItem = dynamic_cast<NotebookStackItem*>(pParentItem);
    if (Q_UNLIKELY(!pStackItem)) {
    ...
    for(int i = 0; i < count; ++i)
    {
        ...
        // Adding wrapping notebook model item
        auto modelItemInsertionResult = m_modelItemsByLocalUid.insert(
            item.localUid(),
            pAddedNotebookItem);
...
bool NotebookModel::removeRows(int row, int count, const QModelIndex & parent)
{
    ...
    if (pParentItem == m_pInvisibleRootItem) {
        QNDEBUG("Can't remove direct child of invisible root item "
                   ...
                   (pParentItem->type() != INotebookModelItem::Type::Stack)))
    {
        QNDEBUG("Can't remove row(s) from parent item not being "
            << "a stack item or all notebooks root item: " << *pParentItem);
        return false;
    }
    ...
    for(int i = 0; i < count; ++i)
    {
        INotebookModelItem * pChildItem = pParentItem->childAtRow(row + i);
        if (!pChildItem) {
            QNWARNING("Detected null pointer to child notebook "
                << "model item on attempt to remove row " << row + i
        ...
        bool isNotebookItem =
            (pChildItem->type() == INotebookModelItem::Type::Notebook);

        if (isNotebookItem) {
            pNotebookItem = dynamic_cast<NotebookItem*>(pChildItem);
        }
        else {
            pStackItem = dynamic_cast<NotebookStackItem*>(pChildItem);
        }
...
// CHECK_NOTEBOOK_ITEM
        ...
        else
        {
            ...
            QList<INotebookModelItem*> notebookModelItemsWithinStack =
                pChildItem->children();
            for(int j = 0, size = notebookModelItemsWithinStack.size();
                j < size; ++j)
            {
                INotebookModelItem * pNotebookModelItem =
                ...
                NotebookItem * pChildNotebookItem =
                    dynamic_cast<NotebookItem*>(pNotebookModelItem);
                if (Q_UNLIKELY(!pChildNotebookItem))
                {
                    QNWARNING("Internal inconsistency detected: notebook model "
                        << "item has notebook type but cannot cast it to "
                        << "NotebookItem: " << pNotebookModelItem);
                    continue;
                }
                ...
                CHECK_NOTEBOOK_ITEM(pChildNotebookItem)
                notebookLocalUidsToRemove << pChildNotebookItem->localUid();
                QNTRACE("Marked notebook local uid "
    ...
    for(int i = 0, size = notebookLocalUidsToRemove.size(); i < size; ++i)
    {
        ...
        {
            INotebookModelItem * pModelItem = *modelItemIt;
            ...
            if (pParentItem && (pParentItem != m_pAllNotebooksRootItem) &&
                (pParentItem->type() == INotebookModelItem::Type::Stack) &&
                (pParentItem->childrenCount() == 0))
            {
                auto * pParentStackItem =
                    dynamic_cast<NotebookStackItem*>(pParentItem);
                if (pParentStackItem)
                {
                    ...
                    {
                        auto * pLinkedNotebookItem =
                            dynamic_cast<NotebookLinkedNotebookRootItem*>(
                                pGrandParentItem);
                        if (pLinkedNotebookItem) {
                            linkedNotebookGuid =
                                pLinkedNotebookItem->linkedNotebookGuid();
                ...
                {
                    REPORT_ERROR(
                        QT_TR_NOOP("Internal inconsistency detected in "
                                   "the notebook model: the model item of "
                                   "stack type cannot be casted to "
                                   "NotebookStackItem"));
                }
            }
        ...
        {
            QNWARNING("Internal error detected: can't find the notebook model "
                << "item corresponding to the notebook item being removed: "
                << "local uid = " << localUid);
        }
        ...
        i < size; ++i)
    {
        ...
        {
            INotebookModelItem * pStackItem = *stackModelItemIt;
            QNTRACE("Notebook stack model item: " << *pStackItem);

            int row = m_pAllNotebooksRootItem->rowForChild(pStackItem);
            QNTRACE("Row for the notebook stack model item within "
                << "all notebooks root item: " << row);

            removeModelItemFromParent(*pStackItem);
        ...
        if (Q_LIKELY(stackItemIt != stackItems.end()))
        {
            ...
            QNTRACE("Erased the notebook stack item corresponding "
                << "to stack " << stack << ", linked notebook guid = "
                << linkedNotebookGuid);
        }
        else
        {
    ...
    QNTRACE("Successfully removed row(s) from notebook model");
    return true;
}
...
QMimeData * NotebookModel::mimeData(const QModelIndexList & indexes) const
{
    ...
    INotebookModelItem * pItem = itemForIndex(indexes.at(0));
    if (!pItem) {
        return nullptr;
    }
    ...
    const QModelIndex & parentIndex)
{
    ...
    INotebookModelItem * pNewParentItem = itemForIndex(parentIndex);
    if (!pNewParentItem) {
    ...
    if (pNewParentItem == m_pInvisibleRootItem) {
        QNDEBUG("Can't drop notebook model items directly onto invisible "
            << "root item");
        return false;
    }
    ...
    NotebookStackItem notebookStackItem;
    INotebookModelItem * pItem = nullptr;

    if (type == static_cast<quint32>(INotebookModelItem::Type::Notebook)) {
        pItem = &notebookItem;
    }
    else if (type == static_cast<quint32>(INotebookModelItem::Type::Stack)) {
        pItem = &notebookStackItem;
    }
    else {
    ...
    if (pItem->type() != INotebookModelItem::Type::Notebook) {
    ...
    auto * pParentLinkedNotebookItem =
        dynamic_cast<NotebookLinkedNotebookRootItem*>(pNewParentItem);
    ...
    INotebookModelItem * pOriginalParentItem = it.value()->parent();
    int originalRow = -1;
    ...
    // FIXME: remove this
    // it = m_modelItemsByLocalUid.insert(notebookItem.localUid(), item);

    beginInsertRows(parentIndex, row, row);

    auto * pNewParentStackItem =
        dynamic_cast<NotebookStackItem*>(pNewParentItem);
    ...
    if (Q_LIKELY(notebookItemIt != localUidIndex.end())) {
        Q_UNUSED(localUidIndex.replace(notebookItemIt, notebookItem))
    }
    else {
        auto insertionResult = localUidIndex.insert(notebookItem);
        notebookItemIt = insertionResult.first;
    }

    INotebookModelItem * pModelItem = const_cast<NotebookItem*>(
        &(*notebookItemIt));
    ...
    LinkedNotebook linkedNotebook, QUuid requestId)
{
    ...
    {
        INotebookModelItem * pModelItem = modelItemIt.value();
        INotebookModelItem * pParentItem = pModelItem->parent();
        if (pParentItem)
        {
    ...
    if (modelItemsByStackIt != m_modelItemsByStackByLinkedNotebookGuid.end()) {
        Q_UNUSED(m_modelItemsByStackByLinkedNotebookGuid.erase(
            modelItemsByStackIt))
    }
    ...
    const INotebookModelItem & item, const Column column) const
{
    ...
    if (isLinkedNotebookItem) {
        pLinkedNotebookItem =
            dynamic_cast<const NotebookLinkedNotebookRootItem*>(&item);
    }

    const NotebookStackItem * pStackItem = nullptr;
    if (!isNotebookItem && !isLinkedNotebookItem) {
        pStackItem = dynamic_cast<const NotebookStackItem*>(&item);
    }

    if (Q_UNLIKELY((isNotebookItem && !pNotebookItem)))
    {
        QNWARNING("Failed to cast notebook model item of notebook type to "
            << "NotebookItem");
        return QVariant();
    }
    else if (Q_UNLIKELY(isLinkedNotebookItem && !pLinkedNotebookItem))
    {
        QNWARNING("Failed to cast notebook model item of linked notebook type "
            << "to NotebookLinkedNotebookItem");
        return QVariant();
    }
    else if (Q_UNLIKELY(isStackItem && !pStackItem))
    {
        QNWARNING("Failed to cast notebook model item of stack type "
            << "to NotebookStackItem");
        return QVariant();
    }
    ...
    case Column::Name:
        {
            if (isNotebookItem) {
                return QVariant(pNotebookItem->name());
            }
            else if (isLinkedNotebookItem) {
                return QVariant(pLinkedNotebookItem->username());
            }
            else {
                return QVariant(pStackItem->name());
            }
        }
    case Column::Synchronizable:
        {
            if (isNotebookItem) {
                return QVariant(pNotebookItem->isSynchronizable());
            }
            else {
                return QVariant();
            }
    ...
    case Column::Dirty:
        {
            if (isNotebookItem) {
                return QVariant(pNotebookItem->isDirty());
            }
            else {
                return QVariant();
            }
    ...
    case Column::Default:
        {
            if (isNotebookItem) {
                return QVariant(pNotebookItem->isDefault());
            }
            else {
                return QVariant();
            }
    ...
    case Column::LastUsed:
        {
            if (isNotebookItem) {
                return QVariant(pNotebookItem->isLastUsed());
            }
            else {
                return QVariant();
            }
    ...
    case Column::Published:
        {
            if (isNotebookItem) {
                return QVariant(pNotebookItem->isPublished());
            }
            else {
                return QVariant();
            }
    ...
    case Column::FromLinkedNotebook:
        {
            if (isNotebookItem) {
                return QVariant(
                    !(pNotebookItem->linkedNotebookGuid().isEmpty()));
            }
            else {
                return QVariant();
            }
    ...
    case Column::NumNotesPerNotebook:
        {
            if (isNotebookItem) {
                return QVariant(pNotebookItem->noteCount());
            }
            else {
                return QVariant();
            }
    ...
    const INotebookModelItem & item, const Column column) const
{
    ...
    QString accessibleText =
        (isNotebookItem ? tr("Notebook: ") : tr("Notebook stack: "));
    ...
    case Column::Dirty:
        {
            ...
            accessibleText +=
                (textData.toBool() ? tr("dirty") : tr("not dirty"));
            break;
        }
    case Column::Default:
        {
...
QString NotebookModel::nameForNewNotebook() const
{
    ...
    return newItemName<NotebookDataByNameUpper>(
        nameIndex,
...
void NotebookModel::onNotebookAdded(const Notebook & notebook)
{
    ...
    if (notebook.hasStack())
    {
        ModelItems * pModelItemsByStack = nullptr;
        StackItems * pStackItems = nullptr;
        INotebookModelItem * pGrandParentItem = nullptr;

        if (!notebook.hasLinkedNotebookGuid())
        {
            pModelItemsByStack = &m_modelItemsByStack;
            pStackItems = &m_stackItems;
            pGrandParentItem = m_pAllNotebooksRootItem;
        }
        else
        {
        ...
        if (it == pModelItemsByStack->end())
        {
            auto stackItemIt = pStackItems->insert(
                stack,
                NotebookStackItem(stack));
    ...
    const NotebookItem * pInsertedItem = &(*it);
    ...
    updateItemRowWithRespectToSorting(*pAddedNotebookItem);
}
    ...
    const Notebook & notebook, NotebookDataByLocalUid::iterator it)
{
    ...
    if (pParentItem->type() == INotebookModelItem::Type::Stack)
    {
        auto * pParentStackItem = dynamic_cast<NotebookStackItem*>(pParentItem);
        if (Q_UNLIKELY(!pParentStackItem)) {
            QNWARNING("Detected null pointer to notebook stack "
    ...
    else if (notebook.hasStack() && (notebook.stack() != previousStackName))
    {
        ...
        if (stackModelItemIt == m_modelItemsByStack.end())
        {
            ...
            INotebookModelItem * pNewStackModelItem =
                const_cast<NotebookStackItem*>(pNewStackItem);
            ...
            int newRow = m_pAllNotebooksRootItem->childrenCount();
            beginInsertRows(
                indexForItem(m_pAllNotebooksRootItem),
                newRow,
                newRow);

            pNewStackModelItem->setParent(m_pAllNotebooksRootItem);
            endInsertRows();

            updateItemRowWithRespectToSorting(*pNewStackModelItem);
        }
    ...
    if (shouldChangeParent)
    {
        ...
        if (Q_UNLIKELY(row < 0)) {
            QNWARNING("Can't find the row for the child notebook item within "
                << "its parent right after setting the parent item to "
                << "the child item! Parent item = " << *pParentItem
                << "\nChild item = " << *pModelItem);
            return;
        }
    ...
    const LinkedNotebook & linkedNotebook)
{
    ...
    auto modelItemIt = m_modelItemsByLinkedNotebookGuid.find(
        linkedNotebookGuid);
    ...
    ModelItems & modelItemsByStack)
{
    ...
    INotebookModelItem * pNewStackModelItem =
        const_cast<NotebookStackItem*>(&stackItem);
...
void NotebookModel::removeItemByLocalUid(const QString & localUid)
{
    ...
    INotebookModelItem * pModelItem = *notebookModelItemIt;
    INotebookModelItem * pParentItem = pModelItem->parent();
    if (Q_UNLIKELY(!pParentItem)) {
        QNWARNING("Can't find the parent notebook model item for "
    ...
    INotebookModelItem & modelItem)
{
    ...
    INotebookModelItem * pParentItem = modelItem.parent();
    if (Q_UNLIKELY(!pParentItem)) {
    ...
    INotebookModelItem & modelItem)
{
    ...
    if (Q_UNLIKELY(currentItemRow < 0)) {
        QNWARNING("Can't update notebook model item's row: can't "
            << "find its original row within parent: " << modelItem);
        return;
    }
...
void NotebookModel::switchDefaultNotebookLocalUid(const QString & localUid)
{
    ...
    if (!m_defaultNotebookLocalUid.isEmpty())
    {
        ...
        if (previousDefaultItemIt != localUidIndex.end())
        {
            ...
            QNTRACE("Previous default notebook item: "
            ...
            QModelIndex previousDefaultItemIndex = indexForLocalUid(
                m_defaultNotebookLocalUid);
...
void NotebookModel::switchLastUsedNotebookLocalUid(const QString & localUid)
{
    ...
    if (!m_lastUsedNotebookLocalUid.isEmpty())
    {
        ...
        if (previousLastUsedItemIt != localUidIndex.end())
        {
            ...
            QModelIndex previousLastUsedItemIndex = indexForLocalUid(
                m_lastUsedNotebookLocalUid);
    ...
    INotebookModelItem & modelItem)
{
    ...
    if (&modelItem == m_pInvisibleRootItem) {
        QNDEBUG("Won't remove the invisible root item");
        return;
    }

    if (&modelItem == m_pAllNotebooksRootItem) {
        QNDEBUG("Won't remove the all notebooks root item");
        return;
    }

    if (modelItem.type() != INotebookModelItem::Type::Stack) {
        QNDEBUG("The model item is not of stack type, won't remove it");
        return;
    }

    const auto * pStackItem = dynamic_cast<const NotebookStackItem*>(&modelItem);
    if (Q_UNLIKELY(!pStackItem)) {
        QNWARNING("Detected notebook model item of stack type but "
            << "cannot cast it to NotebookStackItem: " << modelItem);
        return;
    }
    ...
    if (it != pModelItemsByStack->end())
    {
        ...
        if (stackIt != pStackItems->end()) {
            ...
            QNWARNING("Can't find stack item to remove, stack "
                << previousStack);
        }
    }
    else
    ...
    const QModelIndex & index, const bool favorited)
{
    ...
    if (Q_UNLIKELY(it == localUidIndex.end())) {
        REPORT_ERROR(QT_TR_NOOP("Can't set favorited flag for the notebook: "
                                "the notebook item was not found within "
                                "the model"));
        return;
    }
    ...
    const QString & linkedNotebookGuid)
{
    ...
    NotebookLinkedNotebookRootItem * pLinkedNotebookItem = nullptr;
    auto linkedNotebookItemIt = m_linkedNotebookItems.find(linkedNotebookGuid);
    if (linkedNotebookItemIt == m_linkedNotebookItems.end())
    {
    ...
    INotebookModelItem * pModelItem = linkedNotebookModelItemIt.value();
    int row = rowForNewItem(*m_pAllNotebooksRootItem, *pModelItem);
    beginInsertRows(indexForItem(m_pAllNotebooksRootItem), row, row);
    m_pAllNotebooksRootItem->insertChild(row, pModelItem);
    endInsertRows();
    ...
    const INotebookModelItem & item) const
{
    ...
    {
        auto it = m_indexIdToLocalUidBimap.right.find(pNotebookItem->localUid());
    ...
    else if (isStackItem && pStackItem)
    {
        QString linkedNotebookGuid;
        INotebookModelItem * pParentItem = item.parent();
        if (pParentItem &&
            (pParentItem->type() == INotebookModelItem::Type::LinkedNotebook))
        {
            auto * pLinkedNotebookItem =
    ...
    else if (isLinkedNotebookItem && pLinkedNotebookItem)
    {
