    ...
    QObject(parent), model(_model), fetchingMore(false)
{
    ...
    connect(
        model, SIGNAL(columnsAboutToBeInserted(const QModelIndex &, int, int)),
        this, SLOT(runAllTests()));
    connect(
        model, SIGNAL(columnsAboutToBeRemoved(const QModelIndex &, int, int)),
        this, SLOT(runAllTests()));
    connect(
        model, SIGNAL(columnsInserted(const QModelIndex &, int, int)), this,
        SLOT(runAllTests()));
    connect(
        model, SIGNAL(columnsRemoved(const QModelIndex &, int, int)), this,
        SLOT(runAllTests()));
    connect(
        model, SIGNAL(dataChanged(const QModelIndex &, const QModelIndex &)),
        this, SLOT(runAllTests()));
    connect(
        model, SIGNAL(headerDataChanged(Qt::Orientation, int, int)), this,
        SLOT(runAllTests()));
    connect(model, SIGNAL(layoutAboutToBeChanged()), this, SLOT(runAllTests()));
    connect(model, SIGNAL(layoutChanged()), this, SLOT(runAllTests()));
    connect(model, SIGNAL(modelReset()), this, SLOT(runAllTests()));
    connect(
        model, SIGNAL(rowsAboutToBeInserted(const QModelIndex &, int, int)),
        this, SLOT(runAllTests()));
    connect(
        model, SIGNAL(rowsAboutToBeRemoved(const QModelIndex &, int, int)),
        this, SLOT(runAllTests()));
    connect(
        model, SIGNAL(rowsInserted(const QModelIndex &, int, int)), this,
        SLOT(runAllTests()));
    connect(
        model, SIGNAL(rowsRemoved(const QModelIndex &, int, int)), this,
        SLOT(runAllTests()));

    // Special checks for inserting/removing
    connect(
        model, SIGNAL(layoutAboutToBeChanged()), this,
...
void ModelTest::checkChildren(const QModelIndex & parent, int currentDepth)
{
    ...
    for (int r = 0; r < rows; ++r) {
        ...
        for (int c = 0; c < columns; ++c) {
            ...
            // sign of a bug in data().  Disable if this really is ok in your
            // model.
            //            QVERIFY( model->data ( index, Qt::DisplayRole
            //            ).isValid() );
            ...
            if (model->parent(index) != parent) {
                ...
                //                 And a view that you can even use to show the
                //                 model. QTreeView view; view.setModel(model);
                //                 view.show();
            }
...
void ModelTest::data()
{
    ...
    QVERIFY(
        !model->setData(QModelIndex(), QLatin1String("foo"), Qt::DisplayRole));
    ...
    if (checkStateVariant.isValid()) {
        ...
        QVERIFY(
            state == Qt::Unchecked || state == Qt::PartiallyChecked ||
            state == Qt::Checked);
    }
}
