...
void tst_QMimeDatabase::findByData()
{
    ...
    if (xFail.length() >= 2 && xFail.at(1) == QLatin1Char('x')) {
        // Expected to fail
        QVERIFY2(resultMimeTypeName != mimeTypeName, qPrintable(resultMimeTypeName));
#ifdef Q_OS_WIN    // WARNING from Quentier dev: skipping known to fail tests
...
{
    // WARNING from Quentier dev: these test fails for me on OS X and I don't have enough resources
