 ...
 * @version $Id:$
 */
class Monitor::Impl : public std::condition_variable_any {

public:
    Impl() : ownedMutex_(new Mutex()), mutex_(NULL)
    {
        init(ownedMutex_.get());
    }

    Impl(Mutex * mutex) : mutex_(NULL)
    {
        init(mutex);
    }

    Impl(Monitor * monitor) : mutex_(NULL)
    {
        init(&(monitor->mutex()));
    }
    ...
    int waitForTimeRelative(int64_t timeout_ms)
    {
        ...
        std::timed_mutex* mutexImpl =
                reinterpret_cast<std::timed_mutex*>(mutex_->getUnderlyingImpl());
        assert(mutexImpl);

        std::unique_lock<std::timed_mutex> lock(*mutexImpl, std::adopt_lock);
        int res = (wait_until(lock, std::chrono::system_clock::now() +
    ...
    int waitForTime(const timespec* abstime)
    {
        ...
        int res = (wait_until(lock, std::chrono::system_clock::now() +
                                    ...
                                    std::chrono::microseconds(tv_nsec / 1000)) == std::cv_status::no_timeout
                   ? 0
                   : ETIMEDOUT);
        lock.release();
        return res;
    ...
    int waitForever()
    {
        ...
        std::unique_lock<std::timed_mutex> lock(*mutexImpl, std::adopt_lock);
        ((std::condition_variable_any*)this)->wait(lock);
        lock.release();
        return 0;
    }
...
private:
    ...
    std::unique_ptr<Mutex> ownedMutex_;
    Mutex* mutex_;
};
