...
//     return QModelIndex();
  ...
  QList<QList<qint64> > childIdColumns = m_childItems.value(static_cast<qint64>(parent.internalId()));

  const qint64 grandParent = findParentId(static_cast<qint64>(parent.internalId()));
  if (grandParent >= 0) {
...
QModelIndex DynamicTreeModel::parent(const QModelIndex &index) const
{
  ...
  qint64 searchId = static_cast<qint64>(index.internalId());
...
{
  QList<QList<qint64> > cols = m_childItems.value(static_cast<qint64>(index.internalId()));
...
{
//   Q_UNUSED(index);
  return m_childItems.value(static_cast<qint64>(index.internalId())).size();
}
...
QVariant DynamicTreeModel::data(const QModelIndex &index, int role) const
{
  ...
  {
    return m_items.value(static_cast<qint64>(index.internalId()));
  }
  return QVariant();
}
...
void ModelInsertCommand::doCommand()
{
  ...
  qint64 parentId = static_cast<qint64>(parent.internalId());
...
void ModelMoveCommand::doCommand()
{
    ...
    {
        QList<qint64> l = m_model->m_childItems.value(static_cast<qint64>(srcParent.internalId()))[column].mid(m_startRow, m_endRow - m_startRow + 1 );

        for (int i = m_startRow; i <= m_endRow ; i++)
        {
            m_model->m_childItems[static_cast<qint64>(srcParent.internalId())][column].removeAt(m_startRow);
        }
        int d;
        ...
        {
            m_model->m_childItems[static_cast<qint64>(destParent.internalId())][column].insert(d++, id);
        }
    }
