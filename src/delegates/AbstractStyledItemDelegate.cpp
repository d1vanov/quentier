...
int AbstractStyledItemDelegate::columnNameWidth(const QStyleOptionViewItem & option,
                                                const QModelIndex & index,
                                                const Qt::Orientation orientation) const
{
    const QAbstractItemModel * model = index.model();
    if (Q_UNLIKELY(!model)) {
        QNTRACE(QStringLiteral("Can't determine the column name width: the model "
                               "is null"));
        return -1;
    }

    if (Q_UNLIKELY(!index.isValid())) {
        QNTRACE(QStringLiteral("Can't determine the column name width for invalid "
                               "model index"));
        return -1;
    }
    ...
    if (Q_UNLIKELY(model->columnCount(index.parent()) <= column)) {
        QNTRACE(QStringLiteral("Can't determine the column name width: index's "
                               "column number is beyond the range of model's "
                               "columns"));
        return -1;
    }
    ...
    if (Q_UNLIKELY(columnName.isEmpty())) {
        QNTRACE(QStringLiteral("Can't determine the column name width: model "
